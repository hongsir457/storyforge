...
Storyforge 通过统一的 `ImageBackend` / `VideoBackend` / `TextBackend` 协议，支持多个预置供应商和自定义供应商，可在全局或项目级别切换：
...
Storyforge 的 AI 助手基于 Claude Agent SDK 构建，采用**编排 Skill + 聚焦 Subagent** 的多智能体架构：
...
Storyforge 支持通过 [OpenClaw](https://openclaw.ai) 等外部 AI Agent 平台调用，实现自然语言驱动的视频创作：

1. 在 Storyforge 设置页生成 API Key（`arc-` 前缀，暂保持兼容）
2. 在 OpenClaw 中加载 Storyforge 的 Skill 定义（访问 `http://your-domain/skill.md` 自动获取）
