...
- 仓库内部兼容标识已经从 `arcreel` 统一迁移为 `autovedio`
- 当前常见内部标识包括：
  - SQLite 文件：`projects/.autovedio.db`
  - 项目导出清单：`autovedio-export.json`
  - autonovel 导入脚本：`tools/import_autonovel_to_autovedio.py`
- 如果你在迁移旧数据、搜索日志、改脚本或检查导出包，请优先搜索 `autovedio`
