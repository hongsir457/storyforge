...
Use this when upgrading an existing Storyforge deployment that still stores relational data in `projects/.autovedio.db`.
...
```bash
cp projects/.autovedio.db projects/.autovedio.db.bak
```
...
python scripts/migrate_sqlite_to_postgres.py \
  --source-sqlite projects/.autovedio.db \
...
```bash
sqlite3 projects/.autovedio.db "SELECT 'users', COUNT(*) FROM users UNION ALL SELECT 'tasks', COUNT(*) FROM tasks;"
```
